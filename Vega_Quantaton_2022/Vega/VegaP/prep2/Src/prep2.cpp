                 ...
                 double dInitialTime)
{
    ...
    return [dTheta, dLambda, dSigma,
            dInitialTime](double dT)
    {
        double q = dTheta * ((1 - exp(-dLambda * (dT - dInitialTime))) / (dLambda * (dT - dInitialTime))) + ((dSigma * dSigma) / 2) * ((1 - exp(-dLambda * (dT - dInitialTime))) / (2 * dLambda * (dT - dInitialTime)));
        return q;
    };
}
