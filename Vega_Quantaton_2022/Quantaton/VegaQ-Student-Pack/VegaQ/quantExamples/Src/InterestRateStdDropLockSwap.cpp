...
cfl::Slice NQuant::
    swapRate(unsigned iTime, double dPeriod,
             unsigned iPeriods, const InterestRateModel &rModel)
{
...
cfl::MultiFunction prb::
    dropLockSwap(const Data::Swap &rSwap, double dLowerRate,
                 double dUpperRate, InterestRateModel &rModel)
{
  ...
  Slice uOption = NQuant::swap(iTime, uSwapOnePeriod, rModel);
  ...
  while (iTime > 0)
  {
    ...
    Slice uSwapRate = NQuant::swapRate(iTime, rSwap.period, rSwap.numberOfPayments, rModel);
    uSwap.numberOfPayments = rModel.eventTimes().size() - iTime;
    uSwap.rate = dLowerRate;
    uOption += (NQuant::swap(iTime, uSwap, rModel) - uOption) * indicator(dLowerRate, uSwapRate);
    uSwap.rate = dUpperRate;
    uOption += (NQuant::swap(iTime, uSwap, rModel) - uOption) * indicator(uSwapRate, dUpperRate);
    iTime--;
    uOption.rollback(iTime);
    uOption += NQuant::swap(iTime, uSwapOnePeriod, rModel);
  }
