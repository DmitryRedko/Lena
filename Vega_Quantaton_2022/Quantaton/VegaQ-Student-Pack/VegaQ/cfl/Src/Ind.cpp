...
namespace cflInd
{
  ...
  public:
    ...
    {
      std::transform(begin(rValues), end(rValues), begin(rValues), [dBarrier](double dX) {
        return (dX >= dBarrier) ? 1. : 0.;
      });
    }
  };

  //linear indicator
  std::function<double(double, double)> linearInd(double &rIndL)
  {
    return [&rIndL](double dL, double dR) {
      ...
      {
        rIndL =
            std::abs((std::max(dL, 0.) - std::max(dR, 0.)) / (dL - dR));
      }
      else
      {
        ASSERT(dL == dR);
  ...
  //quadratic indicator
  std::function<double(double, double)> quadInd(double &rIndL)
  {
    return [&rIndL](double dL, double dR) {
      double dInd = rIndL;
      if ((dL < 0.) && (dR >= 0.))
  ...
  public:
    void indicator(std::valarray<double> &rValues, double dBarrier) const
    {
      rValues -= dBarrier;
      double dIndL = (rValues[0] < 0) ? 0. : 1.;
      std::transform(begin(rValues), end(rValues) - 1, begin(rValues) + 1, begin(rValues),
                     quadInd(dIndL));
      //finish at right end
      rValues[rValues.size() - 1] = 0.5 * dIndL;
      if (rValues[rValues.size() - 1] >= 0.)
...
cfl::Ind cfl::NInd::linear()
{
  return Ind(new cflInd::Linear());
}

cfl::Ind cfl::NInd::quadratic()
{
