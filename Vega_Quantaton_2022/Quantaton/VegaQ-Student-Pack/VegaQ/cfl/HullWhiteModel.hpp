...
namespace cfl
{
  ...
  namespace HullWhite
  {
    ...
    public:
      /**
       * Initial discount curve \f$B(t_0,T)\f$, \f$T\geq t_0\f$.
       *
       */
      Function discount;

      /**
       * Initial shape curve \f$A(T)\f$, \f$T\geq t_0\f$, \f$A(t_0) =
      ...
      /**
       * Initial time \f$t_0\f$ given as year fraction.
       *
       */
      double initialTime;
    };
     ...
     *   martingale measure corresponding to maturity \f$T\f$.
     *
     * @param rDiscount Initial discount curve \f$B(t_0,T)\f$,
     * \f$T\geq t_0\f$.
     * @param rVolatility  Average normalized volatility curve
     * \f[
     * \Sigma(T) = \frac1{T-t_0} \sqrt{\int_{t_0}^T \sigma^2(t) dt},
     * \quad T\geq t_0.
     * \f]
     ...
     * written on the discount factor with maturity \f$U>T\f$ is
     * \f$(A(U)-A(T))\Sigma(T)\f$.
     * @param rShape Initial shape curve \f$A(T)\f$, \f$T\geq t_0\f$, \f$A(t_0) =
     *  0\f$, \f$A'(t_0) = 1\f$. It describes the form of changes in
     *  the yield and discount curves.
     ...
     * \f]
     *
     * @param rDiscount Initial discount curve \f$B(t_0,T)\f$, \f$T\geq t_0\f$.
     * @param dKappa The volatility of the spot price.
     * @param dLambda The mean-reversion coefficient of the spot price.
     * @param dInitialTime The initial time given as year fraction.
     */
    Data makeData(const Function &rDiscount, double dKappa,
     ...
     * @param dInterval The interval of initial values for short-term
     * interest rate.
     * @param rBrownian Constructor of Brownian model.
     * @return \p InterestRateModel as Hull and White model.
     */
    InterestRateModel model(const Data &rData, double dInterval, const TBrownian &rBrownian);
  } // namespace HullWhite
  /** @} */
} // namespace cfl
